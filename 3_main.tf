...
resource "aws_launch_template" "asg-lt" {
  ...
  # first db endpoint must be created and retrieved in order to be used in template. Because of that first db endpoint needed to be created then tepmlate will use it
  depends_on = [
...
resource "aws_db_instance" "db-server" {
  ...
  # change the database name, because you have already created the repo with this name
  db_name                     = "phonebook"
  engine                      = "mysql"
